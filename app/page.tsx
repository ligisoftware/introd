...
export default async function Home() {
  return (
    <main className="lp-page">
      ...
      <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:100,backdropFilter:'blur(16px)',WebkitBackdropFilter:'blur(16px)'}}>
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 24px',display:'flex',alignItems:'center',height:68}}>
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <div style={{width:32,height:32,background:'rgba(255,255,255,0.1)',border:'1px solid rgba(255,255,255,0.15)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      {/* ── HERO ── */}
      <section style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',paddingTop:80,position:'relative',overflow:'hidden',background:'#050505'}}>
        {/* Beams 3D background */}
        <BeamsWrapper
          beamWidth={2.5}
          beamHeight={18}
        ...
        <div style={{position:'absolute',inset:0,zIndex:1,background:'linear-gradient(to top,rgba(5,5,5,0.8) 0%,rgba(5,5,5,0.4) 40%,rgba(5,5,5,0.25) 100%)',pointerEvents:'none'}}></div>
        {/* Radial darkening behind text center */}
        <div style={{position:'absolute',inset:0,zIndex:2,background:'radial-gradient(ellipse 80% 60% at 50% 50%,rgba(5,5,5,0.5) 0%,transparent 100%)',pointerEvents:'none'}}></div>

        <div style={{position:'relative',zIndex:10,textAlign:'center',padding:'0 24px',maxWidth:900,margin:'0 auto'}}>
          {/* Badge */}
          <div style={{display:'inline-flex',alignItems:'center',gap:8,marginBottom:28,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:100,padding:'7px 18px'}}>
            <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M8 1L10 6H15L11 9.5L12.5 15L8 12L3.5 15L5 9.5L1 6H6L8 1Z" fill="rgba(255,255,255,0.7)"/></svg>
            <span style={{color:'rgba(255,255,255,0.75)',fontSize:12,fontFamily:"'Sora',sans-serif",fontWeight:500,letterSpacing:'0.01em'}}>Coming Soon</span>
          </div>

          {/* Headline */}
          <h1 style={{fontSize:'clamp(48px,7vw,86px)',fontWeight:900,lineHeight:1.06,letterSpacing:'-0.04em',marginBottom:22,color:'white'}}>
            Raise Faster With<br/>
            <span style={{color:'rgba(255,255,255,0.45)'}}>Warm Introductions</span>
          </h1>

          <p style={{fontSize:18,color:'rgba(255,255,255,0.45)',lineHeight:1.7,marginBottom:80,maxWidth:520,marginLeft:'auto',marginRight:'auto',fontFamily:"'Figtree',sans-serif"}}>
            A single shareable page with your background, startup, and team — AI-scored for the investors and connectors who matter.
          </p>
        </div>

        {/* Dashboard Mockup + Pills — hidden for demo, restore by changing false to true */}
        {(false as boolean) && (<>
        <div className="lp-float" style={{position:'relative',zIndex:10,width:'100%',maxWidth:1000,padding:'0 24px'}}>
          <div className="lp-glass" style={{padding:0,overflow:'hidden',boxShadow:'0 40px 120px rgba(0,0,0,0.8),0 0 0 1px rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.08)'}}>
            <div style={{background:'rgba(10,10,10,0.98)',padding:'12px 16px',borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',gap:8}}>
              ...
              <div style={{flex:1,background:'rgba(255,255,255,0.04)',borderRadius:4,padding:'3px 12px',fontSize:11,color:'rgba(255,255,255,0.2)',maxWidth:280,margin:'0 auto',textAlign:'center',fontFamily:"'Figtree',sans-serif"}}>app.introd.io/dashboard</div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'175px 1fr',minHeight:320,background:'rgba(7,7,7,0.99)'}}>
              <div style={{borderRight:'1px solid rgba(255,255,255,0.05)',padding:16}}>
                <div style={{fontSize:9,color:'rgba(255,255,255,0.2)',fontWeight:700,letterSpacing:'0.1em',marginBottom:12,fontFamily:"'Sora',sans-serif"}}>NAVIGATION</div>
                <div style={{display:'flex',flexDirection:'column',gap:3,marginBottom:20}}>
                  {[{label:'Dashboard',active:true},{label:'My Intros',active:false},{label:'Connections',active:false},{label:'AI Scores',active:false},{label:'Analytics',active:false}].map(item => (
                    <div key={item.label} style={{background:item.active?'rgba(255,255,255,0.07)':'',borderRadius:7,padding:'7px 10px',display:'flex',alignItems:'center',gap:8}}>
                      <div style={{width:5,height:5,borderRadius:'50%',background:item.active?'rgba(255,255,255,0.8)':'rgba(255,255,255,0.15)'}}></div>
                      <span style={{fontSize:12,color:item.active?'rgba(255,255,255,0.9)':'rgba(255,255,255,0.3)',fontWeight:item.active?500:400}}>{item.label}</span>
                    </div>
                  ))}
                </div>
                <div style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:12}}>
                  <div style={{fontSize:9,color:'rgba(255,255,255,0.4)',fontWeight:700,marginBottom:4,fontFamily:"'Sora',sans-serif"}}>SCORE AVG</div>
                  <div style={{fontSize:26,fontWeight:900,color:'white',fontFamily:"'Sora',sans-serif",lineHeight:1}}>8.7</div>
                  <div style={{fontSize:9,color:'rgba(255,255,255,0.25)',marginTop:2}}>This month</div>
                </div>
              </div>
              <div style={{padding:16,display:'flex',flexDirection:'column',gap:12}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                  <div style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:10,padding:12}}>
                    <div style={{fontSize:9,color:'rgba(255,255,255,0.5)',fontWeight:700,fontFamily:"'Sora',sans-serif",marginBottom:4}}>INTRO VIEWS</div>
                    <div style={{fontSize:18,fontWeight:900,color:'white',fontFamily:"'Sora',sans-serif"}}>1,247</div>
                    <div style={{fontSize:9,color:'rgba(100,220,100,0.8)',marginTop:3}}>↑ 18% this week</div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:10,padding:12}}>
                    <div style={{fontSize:9,color:'rgba(255,255,255,0.35)',fontWeight:700,fontFamily:"'Sora',sans-serif",marginBottom:4}}>INTRO REQUESTS</div>
                    <div style={{fontSize:18,fontWeight:900,color:'white',fontFamily:"'Sora',sans-serif"}}>48</div>
                    <div style={{fontSize:9,color:'rgba(100,220,100,0.8)',marginTop:3}}>+5 this week</div>
                  </div>
                  <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:10,padding:12}}>
                    <div style={{fontSize:9,color:'rgba(255,255,255,0.35)',fontWeight:700,fontFamily:"'Sora',sans-serif",marginBottom:4}}>AI SCORE AVG</div>
                    <div style={{fontSize:18,fontWeight:900,color:'white',fontFamily:"'Sora',sans-serif"}}>8.7 / 10</div>
                    <div style={{fontSize:9,color:'rgba(100,220,100,0.8)',marginTop:3}}>↑ 0.3 pts</div>
                  </div>
                </div>
                <div style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:10,padding:12,flex:1}}>
                  <div style={{fontSize:10,color:'rgba(255,255,255,0.25)',fontWeight:700,fontFamily:"'Sora',sans-serif",marginBottom:10}}>INTRODUCTION ACTIVITY</div>
                  <div style={{display:'flex',alignItems:'flex-end',gap:5,height:70}}>
                    {[45,68,52,88,62,47,75,92,58,72,82,100].map((h, i) => (
                      <div key={i} style={{flex:1,background:h>=88?'rgba(255,255,255,0.6)':'rgba(255,255,255,0.15)',borderRadius:'3px 3px 0 0',height:`${h}%`}}></div>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div style={{position:'relative',zIndex:10,display:'flex',flexWrap:'wrap',justifyContent:'center',gap:10,marginTop:28,padding:'0 24px 40px',maxWidth:900}}>
          {[
            {label:'AI-Powered Scoring'},
            {label:'Automated Workflows'},
            {label:'Real-Time Analytics'},
            {label:'Shareable Intro Pages'},
            {label:'Investor Matching'},
          ].map(pill => (
            <div key={pill.label} className="lp-glass" style={{padding:'9px 18px',display:'flex',alignItems:'center',gap:8,borderRadius:100}}>
              <span style={{fontSize:12,fontWeight:600,fontFamily:"'Sora',sans-serif",color:'rgba(255,255,255,0.7)'}}>{pill.label}</span>
            </div>
          ))}
        </div>
        </>)}
      ...
      {/* ── HIDDEN FOR DEMO — set DEMO_MODE = false to restore ── */}
      {(false as boolean) && (<>
      ...
      <section id="features" style={{padding:'120px 0',background:'#0C0E0E',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 24px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:20,marginBottom:20}}>
            {[
              {icon:'/icons/cpu.svg',title:'AI Intro Scoring',desc:'Every intro page gets an AI quality score — so connectors and investors can instantly gauge fit before reaching out.',metric:'AVG SCORE',value:'8.7 / 10',delta:'↑ Top 12% of founders'},
              {icon:'/icons/document-text.svg',title:'Standardized Pages',desc:'One canonical page with your background, startup, team, and pitch deck. No more scattered PDFs or outdated bios.',metric:'COMPLETION',value:'94%',delta:'↑ vs 61% avg'},
              {icon:'/icons/users.svg',title:'Connector Network',desc:'Warm intro requests flow through verified connectors. Every request is tracked, timestamped, and accountable.',metric:'INTROS SENT',value:'3,812',delta:'↑ 24% this month'},
            ].map(card => (
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {[{label:'Deck View Rate',pct:78},{label:'Full Read Rate',pct:52},{label:'Follow-up Rate',pct:34}].map(bar => (
                  <div key={bar.label}>
                    ...
                    <div style={{height:4,borderRadius:2,background:'rgba(255,255,255,0.1)',overflow:'hidden'}}><div style={{height:'100%',borderRadius:2,background:'linear-gradient(90deg,#0D9488,#2DD4BF)',width:`${bar.pct}%`}}></div></div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section id="pricing" style={{padding:'120px 0',background:'#0C0E0E',position:'relative',overflow:'hidden'}}>
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 24px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:20}}>
            {[
              {name:'Free',price:'$0',period:'/month',desc:'Perfect for founders just starting to fundraise.',features:['1 intro page','AI quality score','Shareable link','Basic analytics'],cta:'Get Started Free',highlight:false},
              {name:'Pro',price:'$29',period:'/month',desc:'For active fundraisers who need full visibility.',features:['Unlimited intro pages','Advanced AI scoring','Pitch deck attachments','Detailed analytics','Priority connector access','Custom domain'],cta:'Start Pro Trial',highlight:true},
              {name:'Team',price:'$99',period:'/month',desc:'For studios, accelerators, and cohorts.',features:['Everything in Pro','Up to 20 team members','Bulk intro management','White-label pages','API access','Dedicated support'],cta:'Contact Sales',highlight:false},
            ].map(plan => (
      ...
      <footer style={{borderTop:'1px solid rgba(255,255,255,0.06)',padding:'60px 24px',background:'#080909'}}>
        <div style={{maxWidth:1200,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:24}}>
          ...
          <div style={{display:'flex',gap:24,flexWrap:'wrap'}}>
            <Link href="/feedback" style={{fontSize:13,color:'#566666',textDecoration:'none'}}>Feedback</Link>
            <Link href="/login" style={{fontSize:13,color:'#566666',textDecoration:'none'}}>Log In</Link>
            <a href="mailto:hello@introd.io" style={{fontSize:13,color:'#566666',textDecoration:'none'}}>Contact</a>
          </div>
